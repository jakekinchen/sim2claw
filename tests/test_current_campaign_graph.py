...
def test_current_campaign_graph_is_reproducible_and_backtrackable() -> None:
    ...
    assert tracked["active_pointer"] == {
        "node_id": "checkpoint:canonical-wrist-path-temporal-v1-frozen",
        "milestone_id": "CC02",
        "status": "preregistered_before_exactly_once_wrist_path_dynamic_replay",
        "queue_status": "ACTIVE_CC02_CANONICAL_DYNAMIC_REPLAY",
        "resume_action": (
            "execute_canonical_wrist_path_temporal_v1_exactly_once"
        ),
    ...
    assert tracked["revision_timeline"][-1]["event_id"] == (
        "CANONICAL_WRIST_PATH_TEMPORAL_V1_FROZEN"
    )
    assert tracked["revision_timeline"][-1]["node_ids_added"] == [
        "checkpoint:canonical-wrist-path-temporal-v1-frozen"
    ]

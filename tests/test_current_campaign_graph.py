...
def test_current_campaign_graph_is_reproducible_and_backtrackable() -> None:
    ...
    assert tracked["active_pointer"]["milestone_id"] == "V05-UB"
    assert tracked["active_pointer"] == {
        "node_id": "checkpoint:v05-ub-neighbor-corridor-static-frozen",
        "milestone_id": "V05-UB",
        "status": "preregistered_neighbor_corridor_static_execution_active",
        "queue_status": "ACTIVE_V05_UB_NEIGHBOR_CORRIDOR_STATIC_FROZEN",
        "resume_action": (
            "execute_one_bounded_324_cell_neighbor_corridor_static_enumeration"
        ),
    ...
    assert (
        ...
        == "V05_UB_NEIGHBOR_CORRIDOR_STATIC_FREEZE"
    )
    assert tracked["revision_timeline"][-1]["node_ids_added"] == [
        "checkpoint:v05-ub-neighbor-corridor-static-frozen"
    ]

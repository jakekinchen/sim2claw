...
def test_current_campaign_graph_is_reproducible_and_backtrackable() -> None:
    ...
    assert tracked["active_pointer"] == {
        "node_id": (
            "verdict:calibration-graph-d405-corner-shape-review-v2-admit"
        ),
        "milestone_id": "CC03",
        "status": "reviewed_for_stage_1_execution",
        "queue_status": "ACTIVE_CC03_CORNER_SHAPE_STAGE_1_EXECUTE",
        "resume_action": "execute_corner_shape_v2_stage_1_once",
    ...
    assert tracked["revision_timeline"][-1]["event_id"] == (
        "CALIBRATION_GRAPH_D405_CORNER_SHAPE_SAFETY_PROVENANCE_V2_FROZEN"
    )
    assert tracked["revision_timeline"][-1]["node_ids_added"] == [
        "checkpoint:calibration-graph-d405-corner-shape-safety-provenance-v2-frozen"
    ]

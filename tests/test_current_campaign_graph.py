...
def test_current_campaign_graph_is_reproducible_and_backtrackable() -> None:
    ...
    assert tracked["active_pointer"] == {
        "node_id": "counterexample:v05-sim-rehearsal-v2-key-error",
        "milestone_id": "V05",
        "status": "failed_closed_versioned_v3_orchestration_correction_active",
        "queue_status": "ACTIVE_V05_VERSIONED_REHEARSAL_V3_ORCHESTRATION_CORRECTION",
        "resume_action": "freeze_v3_orchestration_compatibility_correction",
    ...
    assert (
        ...
        == "V05_SIM_REHEARSAL_V2_PREGRID_FAILURE"
    )
    assert tracked["revision_timeline"][-1]["node_ids_added"] == [
        "counterexample:v05-sim-rehearsal-v2-key-error"
    ]

...
MODULE_PATH = (
    ROOT / "tools/evaluate_calibration_graph_d405_wrist_corner_shape_heldout_v5.py"
)
SPEC = importlib.util.spec_from_file_location(
    "evaluate_calibration_graph_d405_wrist_corner_shape_heldout_v5",
    MODULE_PATH,
)
...
def test_v5_runner_exposes_callable_entrypoint() -> None:
    assert callable(MODULE.evaluate)


def test_v5_copy_uses_captured_original_callable() -> None:
    manifest = json.loads(
        (
            ROOT

...
from sim2claw.observable_registration_planar_array_residual_motion_ownership_attribution_identity_reproduction import (
    DEFAULT_OUTPUT,
    evaluate_once,
...
def test_or124b_contract_binds_quarantine_and_new_output() -> None:
    contract = load_identity_reproduction_contract()

    assert contract["experiment_id"] == "OR124B_IDENTITY_BOUND_RESIDUAL_MOTION_OWNERSHIP_REPRODUCTION_V1"
    assert contract["sources"]["or124_quarantined_receipt"]["admitted"] is False
    assert contract["sources"]["or124_closeout"]["status"] == "TERMINAL_IMPLEMENTATION_IDENTITY_DRIFT_QUARANTINED"
...
def test_or124b_final_measurement_logic_recovers_known_translation() -> None:
...
def test_or124b_one_shot_symbol_is_callable() -> None:

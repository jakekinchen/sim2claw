...
EXPERIMENT="${EXPERIMENT:-${SIM2CLAW_ROOT}/configs/experiments/groot_n17_temporal_overlap_v3.json}"
EXPECTED_EXPERIMENT_SHA256="${EXPECTED_EXPERIMENT_SHA256:-a07ea6b50d492511ab98720ae0b1ae44534a8a2fa66eff9e19d56d3d2424e458}"
PARENT_SUMMARY="${PARENT_SUMMARY:-/home/shadeform/runs/groot-n17-consensus/waypoint-v2-development/summary.json}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/shadeform/runs/groot-n17-consensus/temporal-v3-development}"
...
if [[ "$(sha256sum "${EXPERIMENT}" | awk '{print $1}')" != "${EXPECTED_EXPERIMENT_SHA256}" ]]; then
  echo "temporal overlap experiment hash mismatch" >&2
  exit 1
fi
if [[ "$(sha256sum "${PARENT_SUMMARY}" | awk '{print $1}')" != "$(jq -r '.parent_waypoint_summary_sha256' "${EXPERIMENT}")" ]]; then
  echo "parent waypoint summary hash mismatch" >&2
  exit 1
fi
if [[ "$(jq -r '.held_out_may_open' "${PARENT_SUMMARY}")" != "false" ]]; then
  echo "parent waypoint summary unexpectedly opened held-out" >&2
  exit 1
fi
    ...
    while IFS= read -r inference_seed; do
      ...
      env \
        ...
        "${UV_BIN}" run python \
          ...
          --execution-horizon "$(jq -r '.execution_horizon' <<<"${arm}")" \
          --temporal-action-aggregation "$(jq -r '.temporal_action_aggregation' <<<"${arm}")" \
          --temporal-decay "$(jq -r '.temporal_decay' <<<"${arm}")" \
          --host "${SERVER_HOST}" \
          --port "${SERVER_PORT}" \
          --output "${output}" \
          >"${output}.log" 2>&1
...
done < <(jq -c '.development.candidate_arms[]' "${EXPERIMENT}")

env PYTHONPATH="${SIM2CLAW_ROOT}/src" "${UV_BIN}" run python \
  "${SIM2CLAW_ROOT}/scripts/analyze_groot_n17_temporal_overlap.py" \

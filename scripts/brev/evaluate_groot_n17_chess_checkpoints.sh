...
EVAL_STEPS="${EVAL_STEPS:-363}"
SPLIT_LABEL="${SPLIT_LABEL:-held-out}"
...
for step in ${STEPS_TO_EVALUATE}; do
  ...
  log="${RUN_ROOT}/${SPLIT_LABEL}-open-loop-${step}.log"
  if [[ ! -d "${checkpoint}" ]]; then
    echo "checkpoint missing: ${checkpoint}" >&2
    exit 1
  fi
  echo "evaluating checkpoint-${step}"
  "${UV_BIN}" run python "${SIM2CLAW_ROOT}/scripts/brev/run_groot_n17_open_loop_eval.py" \
    --diagnostic-seed "${EVAL_SEED}" \

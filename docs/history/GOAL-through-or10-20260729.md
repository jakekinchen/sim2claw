...
Status: `OR10 BOARD-PLANE CAMERA IMPROVED; EXACT INTRINSICS STILL EXTERNAL`

## Long-horizon alignment successor
...
The earlier retained-data fitting/replay campaign is complete. OR9 freezes the
...
counted attempts all at zero.

OR10 is complete. The prior `2.270 px`
camera number consumed only four manually measured playing corners; its `25`
fit intersections were generated by one homography and were not 25 independent
pixel observations. OR10 binds two already-recorded C922 cohorts with the same
device, `640×480` mode, `30 fps`, and fixed-mount token, then extracts a
reviewed visible interior-board mask directly from the retained pixels. It
compares centered square-pixel zero-distortion, `k1`, and `k1+k2` families
across cohorts. Fourteen visible intersections agree at `0.366 px` RMS and
`1.145 px` maximum. On those actual pixels, the prior model measures
`4.214 px` RMS while the no-refit cross-cohort challenger measures `0.973 px`,
a `76.905%` reduction. The pooled inspection candidate uses
`focal=736.802 px` and vertical FOV `36.084°`. `k1` gives no validation gain;
`k1+k2` gives only `0.279%` and drives `k2` to its bound, so distortion remains
unidentified and the zero-distortion family stays selected.

Because its extraction protocol was developed after inspecting the fit images
and no pristine pose remains, OR10 improves the inspection-only board-plane
camera but does not approve exact intrinsics, distortion, global mapping,
contact, task transfer, or canonical simulator-camera replacement. Studio now
shows the old and refined residuals together with this ceiling.

...
Status: `RP04H_FRESH_ANCHOR_STATIC_V3_FROZEN_PENDING_ONE_RUN`
...
| RP04H | `FRESH_ANCHOR_STATIC_V3_FROZEN_PENDING_ONE_RUN` | Restore the arm from the fresh torque-off postflight pose to the natural anchor without contacting the board or pawns. | The V1 packet stopped before motion because torque-off drift exceeded its exact three-degree start envelope. Fresh preflight is `[-17.055,-57.143,99.121,-7.516,-102.462,1.781] deg`. V3 freezes a 278-row pan-away route from that exact pose with the same dual-scene baseline-preservation gates. | Do not widen the start gate or reuse the consumed authorization. Run static once, then freeze a fresh packet only on pass. |
...
Run the fresh-anchor V3 static safe-return screen exactly once. No full-range
retry or task motion is allowed.

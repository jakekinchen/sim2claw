...
| C6 | `IN_PROGRESS_FREEZE` | Freeze and run the physical realized-action trajectory to simulator outcome replay exactly once. | Bind source/contract/implementation/model/evaluator hashes before the run. Initialize from the frozen D1 state; apply the exact gateway-sent `531 x 6` float32 rows in source order through C4's identified plant and the C5-declared unvalidated baseline. Supply no later camera, observed joint, grasp-mode, release-marker, endpoint, or task-state input. Require the unchanged RP04K task gates, including composable center error `≤6 mm`, upright `≤10 deg`, other-piece displacement `≤5 mm`, height error `≤6 mm`, and settled linear/angular velocity. Numerical and promotable results remain separate. | The one result is immutable. C5 prevents promotion even if numerical gates pass. No successor is admitted because C3A identified no untested cross-episode contact mechanism. |

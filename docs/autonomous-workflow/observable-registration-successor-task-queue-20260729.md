...
Status: `IN_PROGRESS_OR28_BILATERAL_APERTURE_COMPOSITION`
...
| OR28 | `ACTIVE_PROSPECTIVELY_FROZEN_PENDING_RUN` | Test the smallest prior-evidence composition that could close the `232` physical enclosure versus `255` simulated bilateral-contact gap. | Keep the OR18 `6°` spatial/yaw candidate and exact OR19 action/model/timing/reset. Replace only OR19's canonical gripper baseline with the independently fit OR6 aperture offset `0.04948239306868429 rad`. Run exactly once with no contact/object parameter change. Require natural contact, preserve the `36.025 mm` progress gate, and report first bilateral contact, tilt/support consequence, height, collateral, and endpoint outcome. | The OR18 yaw is outcome-informed and remains quarantined; OR6 aperture was independently fit before this run. This composition is frozen before dynamic outcome and cannot approve global mapping, transfer, or a general simulator. |
...
```text
Current state: IN_PROGRESS_OR28_BILATERAL_APERTURE_COMPOSITION
Active card: OR28; one-shot prior-evidence composition frozen before outcome
Completed: immutable predecessor C0-C9; OR0-OR23 and OR26-OR27; OR7C/OR7D and OR24/OR25 not run because their prerequisites failed; camera endpoint REAL-to-SIM 1/1
Evidence: OR26/OR27 show close registered planar endpoints and the material orientation/support split. Internal OR21 evidence shows simulator unilateral contact at 231 but bilateral contact only at 255, while the physical retained evidence marks definite enclosure by sample 232.
...
Next step: run the frozen OR18 spatial plus OR6 aperture composition exactly once, then inspect whether bilateral enclosure and upright consequence move toward the physical source
```

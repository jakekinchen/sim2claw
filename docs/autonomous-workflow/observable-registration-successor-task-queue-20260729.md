...
| OR15 | `IN_PROGRESS` | Quarantine the current OR13 camera from retained replay calibration, compose OR13 board/pawn/base geometry with the same-session OR10 camera and the already-frozen OR11 shoulder candidate, then run one exact kinematic contact-phase gate. | Camera and joint values are frozen before the task window opens; no refit, task row, outcome, dynamics, integration, or action change. Report no-refit static pixel metrics and require no precontact through `224` plus named jaw/pawn contact during `228–232` before any dynamics. | Contract `configs/evaluations/observable_registration_retained_camera_composition_v1.json`; output pending. |
...
```text
Current state: IN_PROGRESS_OR15_RETAINED_CAMERA_COMPOSITION
Active card: OR15
Completed: immutable predecessor C0-C9; OR0-OR13; OR7C/OR7D not run because their prerequisite failed; camera endpoint REAL-to-SIM 1/1
Evidence: OR13 artifact 91a7c68e743d13532ecb7a796b53970f3b877efc917ce15b86db8b9db15e2474; 40.5 mm squares; 324 mm playing side; 34 mm compiled pawn; camera-center planar range reconciliation residual 4.422 mm; camera height 305 mm; compiled camera position error 0; retrospective orientation validation 6.857 px RMS; sample-232 fixed-jaw gap 45.473 mm; 0 named contacts; 0 dynamics
Remaining: OR15 must determine whether the prior same-session static shoulder candidate plus new static geometry reaches the physical contact phase; exact intrinsics and pristine heldout extrinsics remain unapproved
...
Next step: execute the frozen OR15 no-refit composition and exact kinematic contact-phase gate; run dynamics only under a separately frozen successor if named contact passes
```

...
| OR41 | `ACTIVE` | Determine whether the already-retained contact-free full-range recording supplies the missing bidirectional gripper excitation without collecting new data. | Bind the full-range recording and D405 stream prospectively before quantitative extraction. First verify that both red jaw surfaces are observable through the complete opening and closing sweeps while the jaw is clear of all objects. If admitted, reuse OR40's scale-free play estimand and deterministic extraction with disjoint direction-stratified fit/validation blocks, then no-refit test the fitted mapping on the untouched OR40 task-clip observations. Keep metric aperture, camera calibration, force/compliance, task outcomes, and simulator replay sealed. If the cross-episode mapping passes, freeze a separate one-replay successor; OR41 itself performs no dynamics. | Existing-source audit only: `full-range-of-motion-capture__20260727T041940Z-5b15dc6e` contains `2401` measured rows, a `642`-frame `5 Hz` D405 browser stream, and one `1.07→99.52→1.66` gripper cycle. No quantitative pad mapping or candidate selection has been opened. |
...
```text
Current state: ACTIVE_RETAINED_FULL_RANGE_GRIPPER_MAPPING
Active card: OR41 retained contact-free full-range gripper mapping
...
Remaining: full matching task outcome, exact intrinsics, pristine heldout extrinsics, globally approved mapping, and physical pawn/contact mechanics remain unapproved. OR40 proves the task clip has image-space pad observability but cannot identify directional play: the null model wins and the validation split has no closing branch.
...
Next step: prospectively audit the already-retained full-range recording for a contact-free opening/closing jaw-surface cycle. If it supplies directionally balanced scale-free observations, fit there and validate without refit on the untouched OR40 task-clip observations; otherwise close the retained-data mapping lane and require the exact load-side sensing experiment named by the OR40 closeout.
```

...
```text
Current state: IN_PROGRESS_OR14_BELIEF_RECALCULATION
Active card: OR14
...
Remaining: OR14 must determine whether any bounded robot-side family is stable under the recalculated OR13 geometry; camera exact intrinsics and pristine heldout extrinsics remain unapproved; global mapping and matching action-to-task outcome remain false
...
Next step: execute the frozen OR14 static image-only factor recalculation, then freeze at most one accepted family for an exact kinematic contact-phase gate; do not use the sample-232 residual as a fit vector
```

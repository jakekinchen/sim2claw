...
| OR40 | `ACTIVE` | Derive the physical gripper encoder-to-contact-surface mapping from the retained synchronized home-workspace footage before another simulator correction. | Use existing video and raw telemetry only. Resolve fixed/moving jaw edges, pawn silhouette, and board scale over the no-contact, enclosure, lift, and carry intervals. Emit per-frame aperture/contact-surface observations with uncertainty and an explicit synchronization binding. Fit at most one gauge-fixed encoder-to-jaw mapping on preterminal observations, preserve a no-refit validation slice, and require residual, rank, conditioning, and bound-margin gates before one exact dynamic replay. Do not use terminal D2 position, inject object states, alter arm measured states, or invent compliance. | Preliminary bounded audit: the `640×480@30 Hz` C922 view is arm/cable occluded at contact, but the retained `424×240@5 Hz` D405 wrist RGB stream clearly resolves both red jaw-tip surfaces, the pawn silhouette between them, and board squares through frames `95–104` (approximately source samples `210–252`). Its exact browser-video SHA is `2d0cf208465d984afc9a601da305e8ce1d94ddba124d05674ec18d5df1be830c`; exposure sync and metric depth remain unavailable. Continue with image-space extraction and an uncertainty-aware mapping, not a metric-depth claim. |
...
```text
Current state: ACTIVE_RETAINED_VIDEO_JAW_SURFACE_MAPPING
Active card: OR40 retained-video encoder-to-contact-surface mapping
...
Remaining: full matching task outcome, exact intrinsics, pristine heldout extrinsics, globally approved mapping, and physical pawn/contact mechanics remain unapproved. OR35-OR39 isolate the residual to gripper encoder-to-contact-surface mapping and unmeasured contact deformation rather than the raw arm trace.
...
Next step: audit whether the retained home-workspace video resolves both physical jaw surfaces and pawn silhouette with enough scale and synchronization to identify OR40; if it does, fit one preterminal mapping and replay it once, otherwise close with the exact missing observation rather than tune another simulator outcome
```

...
```text
Current state: COMPLETE_OR13_STATIC_GEOMETRY_AND_CAMERA_CENTER_CANDIDATE_NO_CONTACT
Active card: none
...
Remaining: direct robot-base height or articulated wrist vertical registration is absent; camera exact intrinsics and heldout extrinsics remain unapproved; base yaw is frozen but unapproved; global mapping and matching action-to-task outcome remain false
External boundary: a new independent robot vertical/base-height or multi-pose articulated registration measurement is required before one further robot mechanism can be declared; hardware authority remains false
Next step: measure the table/board-top-to-left-base-origin height or obtain the already-frozen no-contact multi-pose endpoint observations; do not use the sample-232 residual as a correction vector
```

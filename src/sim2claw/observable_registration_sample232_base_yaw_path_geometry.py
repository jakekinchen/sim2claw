"""Static base-yaw refinement requiring both approach and enclosure geometry."""
...
from .observable_registration_sample232_base_xy_aperture_refinement import load_contract as load_or31_contract
...
SCHEMA = "sim2claw.observable_registration_sample232_base_yaw_path_geometry_contract.v1"
RECEIPT_SCHEMA = "sim2claw.observable_registration_sample232_base_yaw_path_geometry_receipt.v1"
CONTRACT_PATH = REPO_ROOT / "configs/evaluations/observable_registration_sample232_base_yaw_path_geometry_v1.json"
OUTPUT_DIRECTORY = REPO_ROOT / "outputs/observable_registration_sample232_base_yaw_path_geometry_v1"
...
def load_contract(path: Path = CONTRACT_PATH, *, root: Path = REPO_ROOT) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    contract = load_json_object(path, label="sample-232 base yaw path geometry")
    ...
    _, _, or28, c6 = load_or31_contract(
        _bound_path(contract["sources"]["or31_contract"], root=root, label="OR31"),
        root=root,
    )
    _require(
    ...
    _require(
        ...
        "static yaw evaluation widened",
    )
    return contract, or28, c6


def evaluate_base_yaw_path_geometry(
    *, contract_path: Path = CONTRACT_PATH, output_directory: Path = OUTPUT_DIRECTORY, root: Path = REPO_ROOT
) -> dict[str, Any]:
    receipt_path = output_directory / "receipt.json"
    _require(not receipt_path.exists(), "OR32 receipt already exists")
    ...
    original_quat = model.body_quat[left_base].copy()
    ...
    for dx in contract["grid"]["left_base_world_x_delta_m"]:
        for dy in contract["grid"]["left_base_world_y_delta_m"]:
            ...
            for yaw in contract["grid"]["left_base_yaw_delta_degrees"]:
                model.body_quat[left_base] = _multiply_quaternion(_yaw_quaternion(yaw), original_quat)
                for offset in contract["grid"]["gripper_zero_offset_rad"]:
                    ...
                    rows.append({
                        ...
                        "left_base_yaw_delta_degrees": float(yaw),
    ...
    selected = min(eligible, key=lambda row: (row["score"], abs(row["left_base_yaw_delta_degrees"]), row["translation_norm_m"], abs(row["gripper_zero_offset_rad"]))) if eligible else None
    receipt = {
        "schema_version": RECEIPT_SCHEMA,
        "experiment_id": contract["experiment_id"],
        "proof_class": contract["proof_class"],
        "status": "PASS_STATIC_BASE_YAW_PATH_CANDIDATE" if selected else "NO_STATIC_BASE_YAW_PATH_CANDIDATE",

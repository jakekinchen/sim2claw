"""Evaluate the frozen full-mesh renderer across all four development timelines."""
...
DEFAULT_CONTRACT = REPO_ROOT / "configs/evaluations/observable_registration_native_full_mesh_development_timeline_v1.json"
DEFAULT_OUTPUT = REPO_ROOT / "outputs/observable_registration_native_full_mesh_development_timeline_v1"
...
def evaluate_once(
    ...
    if receipt_path.exists():
        raise ValueError("OR80 one-run receipt already exists")
    ...
    or74 = json.loads(
        (REPO_ROOT / contract["sources"]["or74_contract"]["path"]).read_text()
    )
    if any(
        contract[key] != or74[key] for key in ("timeline", "metric", "acceptance")
    ):
        raise ValueError("OR74 timeline, metric, or acceptance drifted")
    or73 = json.loads(
    ...
    if selected["vector"] != contract["selected_camera"]["vector"]:
        raise ValueError("OR73 camera vector drifted")
    camera = {
    ...
    reference = cv2.imread(
        str(REPO_ROOT / contract["sources"]["or79_equivalence_frame"]["path"]),
        cv2.IMREAD_COLOR,
    )
    if reference is None:
        raise ValueError("OR79 equivalence frame unavailable")
    ...
    for episode in episodes:
        ...
        if not writer.isOpened():
            raise RuntimeError("cannot open OR80 candidate video writer")
        ...
        try:
            for slot, (time_seconds, trace_index, video_index, physical) in enumerate(
                ...
                pixels, depths, colors, stream = _prepare_triangle_stream(
                    scene, one_frame_trace, meshes, camera, renderer
                )
                ...
                triangle_counts.append(int(stream["total_raster_triangle_count"]))
                if (
                    recording_id
                    == contract["sources"]["or79_equivalence_frame"]["recording_id"]
                    and slot == 0
                ):
    ...
    integrity_gates = {
        ...
        "first_bound_episode_frame_equal_to_or79": reference_checked
        and reference_matched,
        "validation_and_heldout_closed": True,
        "no_fits_replays_or_paid_compute": True,
    }
    rows_artifact: dict[str, Any] = {
        "schema_version": "sim2claw.observable_registration_native_full_mesh_development_timeline_rows.v1",
        "rows": all_rows,
    }
    ...
    receipt: dict[str, Any] = {
        "schema_version": "sim2claw.observable_registration_native_full_mesh_development_timeline_receipt.v1",
        "experiment_id": contract["experiment_id"],
        "status": (
            "PASS_NATIVE_FULL_MESH_DEVELOPMENT_PIXEL_TARGET"
            if passed
            else "TERMINAL_NATIVE_FULL_MESH_DEVELOPMENT_BELOW_TARGET"
        ),
        ...
        "reviewer_decision": (
            "OPEN_REJECT_ONLY_VALIDATION_WITH_FROZEN_RENDERER"
            if passed
            else "KEEP_VALIDATION_CLOSED_AND_SELECT_DEVELOPMENT_RESIDUAL_MECHANISM"
        ),
        "next_transition": (
            "freeze_or81_reject_only_validation_full_mesh_timeline"
            if passed
            else "freeze_or81_full_mesh_development_residual_attribution"
        ),
    }

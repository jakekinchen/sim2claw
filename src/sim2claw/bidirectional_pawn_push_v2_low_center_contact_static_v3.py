"""V05-UA v3 low-center contact static enumerator with exact quarantine."""
...
class LowCenterContactStaticV3Error(RuntimeError):
    """The prospectively frozen V05-UA v3 static search failed closed."""
...
def _resolve(path: Path) -> Path:
    ...
    try:
        ...
        raise LowCenterContactStaticV3Error(
            "V05-UA v3 path escapes repository"
        ) from error
    return resolved
...
def _verify(entry: Mapping[str, Any]) -> Path:
    ...
    if not path.is_file() or _sha(path) != entry["sha256"]:
        raise LowCenterContactStaticV3Error(
            f"bound V05-UA v3 input changed: {path}"
        )
    return path
...
def _named_id(
    ...
    if object_id < 0:
        raise LowCenterContactStaticV3Error(
            f"required V05-UA v3 object is missing: {name}"
        )
    ...
    for target in dense_cartesian:
        ...
        if residual > maximum_ik_residual_m:
            raise _rehearsal.PushRehearsalError(
                "V05-UA v3 multi-start IK residual exceeded gate"
            )
        seed = np.asarray(
            [
                pose["shoulder_pan"],
...
def enumerate_and_freeze(
    ...
    if (
        ...
        != "sim2claw.bidirectional_pawn_push_v2_low_center_contact_static.v3"
    ):
        raise LowCenterContactStaticV3Error(
            "unexpected V05-UA v3 static contract"
        )
    ...
    if authorization["quarantine"]["case_ids"] != contract["quarantine"][
        ...
        raise LowCenterContactStaticV3Error("V05-UA v3 quarantine changed")
    quarantine = set(contract["quarantine"]["case_ids"])
    if len(quarantine) != int(contract["quarantine"]["exact_count"]):
        raise LowCenterContactStaticV3Error(
            "V05-UA v3 quarantine count binding changed"
        )
    if len(quarantine) != 12:
        raise LowCenterContactStaticV3Error("V05-UA v3 quarantine is not exact")
    ...
    if len(complete_universe) != int(
        ...
        raise LowCenterContactStaticV3Error(
            "V05-UA v3 reset-layout family universe changed"
        )
    universe = [
    ...
    if len(universe) != int(
        ...
        raise LowCenterContactStaticV3Error(
            "V05-UA v3 post-quarantine family universe changed"
        )
    ...
    if closed_jaw != -0.1727003294848389:
        raise LowCenterContactStaticV3Error("V05-UA v3 jaw scalar changed")
    ...
    receipt = {
        "schema_version": (
            "sim2claw."
            "bidirectional_pawn_push_v2_low_center_contact_static_receipt.v3"
        ),
        "status": (
            "low_center_contact_static_freeze_pass"
            if passed
            else "low_center_contact_static_freeze_reject"
        ),
        "proof_class": (
...
__all__ = ["LowCenterContactStaticV3Error", "enumerate_and_freeze"]

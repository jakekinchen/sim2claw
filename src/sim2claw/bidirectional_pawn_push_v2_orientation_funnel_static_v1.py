"""V05-UC fixed-wrist open-loop guiding-contact static enumerator."""
...
from .grasp import _pinch_offset
from .paths import REPO_ROOT
from .physical_canary import _physical_to_model_position
from .scene import board_square_center
from .bidirectional_pawn_push_v2_wrist_path_static import (
    _solve_fixed_wrist_position,
)


class OrientationFunnelStaticV1Error(RuntimeError):
    """The prospectively frozen V05-UC static search failed closed."""
...
def _resolve(path: Path) -> Path:
    ...
    try:
        ...
        raise OrientationFunnelStaticV1Error(
            "V05-UC path escapes repository"
        ) from error
    return resolved
...
def _verify(entry: Mapping[str, Any]) -> Path:
    ...
    if not path.is_file() or _sha(path) != entry["sha256"]:
        raise OrientationFunnelStaticV1Error(
            f"bound V05-UC input changed: {path}"
        )
    return path
...
def _named_id(
    ...
    if object_id < 0:
        raise OrientationFunnelStaticV1Error(
            f"required V05-UC object is missing: {name}"
        )
    ...
    for target in dense_cartesian:
        ...
        if residual > maximum_ik_residual_m:
            raise _rehearsal.PushRehearsalError(
                "V05-UC fixed-wrist funnel IK residual exceeded gate"
            )
        seed = np.concatenate(
            (np.asarray(arm, dtype=np.float64), [closed_jaw_rad])
        )
    ...
    return action, {
        ...
        "maximum_wrist_roll_target_error_rad": float(
            np.max(
                np.abs(
                    action[
                        1 + setup_source_to_branch_samples :, 4
                    ]
                    - wrist_roll_rad
                )
            )
...
def enumerate_and_freeze(
    ...
    if (
        ...
        != "sim2claw.bidirectional_pawn_push_v2_orientation_funnel_static.v1"
    ):
        raise OrientationFunnelStaticV1Error(
            "unexpected V05-UC static contract"
        )
    ...
    if authorization["quarantine"]["case_ids"] != contract["quarantine"][
        ...
        raise OrientationFunnelStaticV1Error("V05-UC quarantine changed")
    quarantine = set(contract["quarantine"]["case_ids"])
    if len(quarantine) != int(contract["quarantine"]["exact_count"]):
        raise OrientationFunnelStaticV1Error(
            "V05-UC quarantine count binding changed"
        )
    if len(quarantine) != 16:
        raise OrientationFunnelStaticV1Error("V05-UC quarantine is not exact")
    ...
    if len(complete_universe) != int(
        ...
        raise OrientationFunnelStaticV1Error(
            "V05-UC reset-layout family universe changed"
        )
    universe = [
    ...
    if len(universe) != int(
        ...
        raise OrientationFunnelStaticV1Error(
            "V05-UC post-quarantine family universe changed"
        )
    ...
    if closed_jaw != -0.1727003294848389:
        raise OrientationFunnelStaticV1Error("V05-UC jaw scalar changed")
    ...
    guide_offsets = contract["parameter_grid"]["guide_lateral_offsets_m"]
    ...
    for family_index, case in enumerate(universe):
        ...
        for branch_index, (branch_spec, branch_model) in enumerate(
            ...
            for wrist_index, wrist_roll in enumerate(wrist_grid):
                for guide_index, guide_offset in enumerate(guide_offsets):
                    guide_contact = (
                        contact + perpendicular * float(guide_offset)
                    )
                    guide_overhead = guide_contact.copy()
                    guide_overhead[2] = (
                        source_xyz[2] + clearance_height
                    )
                    cartesian_waypoints = [
                        guide_overhead,
                        guide_contact,
                        contact,
                        contact + direction * stroke,
                    ]
                    base = {
                        "case_id": case_id,
                        "family_id": case_id,
                        ...
                        **case,
                        ...
                        "guide_lateral_offset_m": guide_offset,
                        "guide_side_index": guide_index,
                    ...
                    source_body_delta = float(
                        np.max(
                            np.abs(
                                first_physical[:5] - source_expected[:5]
                            )
                        )
                    )
                    ...
                    checks = {
                        ...
                        "source_gripper_envelope": (
                            source_gripper_delta
                            <= source_gripper_tolerance
                        ),
    ...
    if expected_grid_count != int(
        contract["parameter_grid"]["maximum_total_cells"]
    ) or len(grid_results) != expected_grid_count:
        raise OrientationFunnelStaticV1Error(
            "V05-UC finite static grid count changed"
        )
        ...
        eligible_cases.append(
            {
                ...
                "guide_lateral_offset_m": row[
                    "guide_lateral_offset_m"
                ],
                "contact_center_offset_m": contact_offset,
                "contact_height_m": contact_height,
                "stroke_m": stroke,
    ...
    receipt = {
        "schema_version": (
            "sim2claw."
            "bidirectional_pawn_push_v2_orientation_funnel_static_receipt.v1"
        ),
        "status": (
            "orientation_funnel_static_freeze_pass"
            if passed
            else "orientation_funnel_static_freeze_reject"
        ),
        "proof_class": (
            "cpu_fp64_static_fixed_wrist_open_loop_guiding_contact_"
            "collision_camera_gateway_action_freeze_only_v1"
        ),
        ...
        "selected_pawn_grasped": False,
        "closed_loop_or_feedback": False,
        "authority": contract["authority"],
        "claim_boundary": (
            "Static-only deterministic fixed-wrist open-loop guiding-contact "
            "search with setup included in exact action bytes. "
...
__all__ = ["OrientationFunnelStaticV1Error", "enumerate_and_freeze"]

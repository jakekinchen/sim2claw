"""Produce one full-step E2-to-E1 exact-action baseline trace for OR148."""
...
CONTRACT_PATH = REPO_ROOT / "configs" / "evaluations" / "pawn_bg_e2_strict_baseline_v1.json"
OUTPUT_ROOT = REPO_ROOT / "outputs" / "pawn_bg_e2_strict_baseline_v1"
SCHEMA = "sim2claw.pawn_bg_e2_strict_baseline.v1"
PRODUCER_SCHEMA = "sim2claw.pawn_bg_e2_strict_baseline_producer.v1"


class E2StrictBaselineError(RuntimeError):
    """The frozen E2 baseline cannot run after provenance drift."""
...
def _validate_binding(binding: Mapping[str, Any]) -> None:
    ...
    if not path.is_file() or sha256_file(path) != str(binding["sha256"]):
        raise E2StrictBaselineError(f"source binding drifted: {binding['path']}")


def load_raw_contract(path: Path = CONTRACT_PATH) -> dict[str, Any]:
    raw = json.loads(path.read_text(encoding="utf-8"))
    if raw.get("schema_version") != SCHEMA:
        raise E2StrictBaselineError("unexpected OR148 schema")
    ...
    if any(raw.get("authority", {}).values()):
        raise E2StrictBaselineError("OR148 authority widened")
    expected_action = {
        "shape": [418, 6],
        "dtype": "float64",
        "c_contiguous": True,
    ...
    if raw["action_invariance"] != expected_action:
        raise E2StrictBaselineError("E2 action invariance drifted")
    if [row["candidate_id"] for row in raw["candidate_order"]] != [
        "e2_rank02_rigid_baseline"
    ]:
        raise E2StrictBaselineError("E2 candidate order drifted")
    if raw["candidate_order"][0].get("parameter_overrides") != {}:
        raise E2StrictBaselineError("OR148 parameter change is forbidden")
    return raw
    ...
    contract.update(
        {
            key: raw[key]
            for key in (
                "schema_version", "experiment_id", "status", "proof_class",
                "authorization", "base_contract", "action_invariance",
                "candidate_order", "e2_parameters", "supplemental_gates",
                "implementation_bindings", "execution", "authority", "claim_boundary",
            )
        }
    )
...
def run_baseline(
    *, output_directory: Path = OUTPUT_ROOT, contract_path: Path = CONTRACT_PATH
    ...
    probe = run_grasp_episode_probe(
        ...
        parameters=copy.deepcopy(contract["e2_parameters"]),
    ...
    if (
        ...
        raise E2StrictBaselineError("exact E2 action identity drifted")
    ...
    receipt = {
        ...
        "parameter_digest": canonical_digest(contract["e2_parameters"]),
...
__all__ = ["E2StrictBaselineError", "load_contract", "load_raw_contract", "run_baseline"]

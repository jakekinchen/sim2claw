"""Prospective V05-TX alternate-start, multi-IK, approach static freezer."""
...
class MultistartApproachStaticError(RuntimeError):
    """The prospectively frozen V05-TX static search failed closed."""
...
def _resolve(path: Path) -> Path:
    ...
    try:
        ...
        raise MultistartApproachStaticError(
            "V05-TX path escapes repository"
        ) from error
    return resolved
...
def _verify(entry: Mapping[str, Any]) -> Path:
    ...
    if not path.is_file() or _sha(path) != entry["sha256"]:
        raise MultistartApproachStaticError(
            f"bound V05-TX input changed: {path}"
        )
    return path
...
def _named_id(
    ...
    if object_id < 0:
        raise MultistartApproachStaticError(
            f"required V05-TX object is missing: {name}"
        )
    ...
    for target in dense_cartesian:
        ...
        if residual > maximum_ik_residual_m:
            raise _rehearsal.PushRehearsalError(
                "V05-TX multi-start IK residual exceeded gate"
            )
        seed = np.asarray(
            [
                pose["shoulder_pan"],
...
def enumerate_and_freeze(
    ...
    if (
        ...
        != "sim2claw.bidirectional_pawn_push_v2_multistart_approach_static.v1"
    ):
        raise MultistartApproachStaticError(
            "unexpected V05-TX static contract"
        )
    ...
    if authorization["quarantine"]["case_ids"] != contract["quarantine"][
        ...
        raise MultistartApproachStaticError("V05-TX quarantine changed")
    quarantine = set(contract["quarantine"]["case_ids"])
    if len(quarantine) != 4:
        raise MultistartApproachStaticError("V05-TX quarantine is not exact")
    ...
    if len(complete_universe) != int(
        ...
        raise MultistartApproachStaticError(
            "V05-TX reset-layout family universe changed"
        )
    universe = [
    ...
    if len(universe) != int(
        ...
        raise MultistartApproachStaticError(
            "V05-TX post-quarantine family universe changed"
        )
    ...
    if closed_jaw != -0.1727003294848389:
        raise MultistartApproachStaticError("V05-TX jaw scalar changed")
    ...
    receipt = {
        "schema_version": (
            "sim2claw."
            "bidirectional_pawn_push_v2_multistart_approach_static_receipt.v1"
        ),
        "status": (
            "multistart_approach_static_freeze_pass"
            if passed
            else "multistart_approach_static_freeze_reject"
        ),
        "proof_class": (
            "cpu_fp64_static_alternate_start_multistart_ik_approach_"
        ...
        "claim_boundary": (
            "Static-only deterministic alternate-start, multi-IK, and "
            "approach-path search with setup included in exact action bytes. "
...
__all__ = ["MultistartApproachStaticError", "enumerate_and_freeze"]

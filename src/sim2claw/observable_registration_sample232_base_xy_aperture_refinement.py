"""One static local refinement of the OR30 sample-232 enclosure basin."""
...
from .observable_registration_sample232_base_xy_aperture_geometry import load_contract as load_or30_contract
...
SCHEMA = "sim2claw.observable_registration_sample232_base_xy_aperture_refinement_contract.v1"
RECEIPT_SCHEMA = "sim2claw.observable_registration_sample232_base_xy_aperture_refinement_receipt.v1"
CONTRACT_PATH = REPO_ROOT / "configs/evaluations/observable_registration_sample232_base_xy_aperture_refinement_v1.json"
OUTPUT_DIRECTORY = REPO_ROOT / "outputs/observable_registration_sample232_base_xy_aperture_refinement_v1"
...
def load_contract(path: Path = CONTRACT_PATH, *, root: Path = REPO_ROOT) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    contract = load_json_object(path, label="sample-232 base XY aperture refinement")
    ...
    or30, or29, or28, c6 = load_or30_contract(
        _bound_path(contract["sources"]["or30_contract"], root=root, label="OR30"),
        root=root,
    )
    _require(
        len(contract["grid"]["left_base_world_x_delta_m"]) == 11
        and len(contract["grid"]["left_base_world_y_delta_m"]) == 11
        and len(contract["grid"]["gripper_zero_offset_rad"]) == 15,
        "refinement grid changed",
    )
    _require(
        contract["evaluation"]["physics_integration_allowed"] is False
        and contract["evaluation"]["dynamic_replay_allowed"] is False
        and contract["evaluation"]["dynamic_outcomes_may_not_select_grid_row"] is True,
        "static refinement widened",
    )
    _require(
        contract["authority"]["simulator_static_evaluation"] is True
        and not any(value for name, value in contract["authority"].items() if name != "simulator_static_evaluation"),
        "authority widened",
    )
...
def evaluate_refinement(
    *, contract_path: Path = CONTRACT_PATH, output_directory: Path = OUTPUT_DIRECTORY, root: Path = REPO_ROOT
    ...
    _require(not receipt_path.exists(), "OR31 receipt already exists")
    contract, _, or28, c6 = load_contract(contract_path, root=root)
    source = c6["source"]
    ...
    selected = min(eligible, key=lambda row: (row["score"], row["translation_norm_m"], abs(row["gripper_zero_offset_rad"]))) if eligible else None
    receipt = {
        "schema_version": RECEIPT_SCHEMA,
        "experiment_id": contract["experiment_id"],
        "proof_class": contract["proof_class"],
        "status": "PASS_STATIC_REFINED_ENCLOSURE_CANDIDATE" if selected else "NO_STATIC_REFINED_ENCLOSURE_CANDIDATE",

"""Post-final renderer-native diagnostic for a separate shared robot-base SE(2)."""
...
SCHEMA = "sim2claw.observable_registration_post_final_shared_robot_base_se2_diagnostic_contract.v1"
DEFAULT_CONTRACT = REPO_ROOT / "configs/evaluations/observable_registration_post_final_shared_robot_base_se2_diagnostic_v1.json"
DEFAULT_OUTPUT = REPO_ROOT / "outputs/observable_registration_post_final_shared_robot_base_se2_diagnostic_v1"


def load_post_final_shared_robot_base_se2_diagnostic_contract(
    path: Path = DEFAULT_CONTRACT,
) -> dict[str, Any]:
    contract = json.loads(path.read_text())
    if contract.get("schema_version") != SCHEMA:
        raise ValueError("unsupported OR93 contract")
    ...
    if len(samples) != 6 or len({(row["recording_id"], row["sample"]) for row in samples}) != 6:
        raise ValueError("OR93 six-sample boundary drifted")
    family = contract["robot_registration_family"]
    if (
        len(family["parameter_names"]) != 3
        or len(family["bounds"]) != 3
        or family["per_episode_parameters"] != 0
        or family["shared_across_both_robot_subtrees_and_all_six_frames"] is not True
    ):
        raise ValueError("OR93 shared three-parameter family drifted")
    frozen = contract["frozen_candidate"]
    static_ids = set(frozen["static_workcell_transform"]["transformed_body_ids"])
    robot_ids = set(frozen["baseline_robot_transform"]["transformed_body_ids"])
    if static_ids & robot_ids or robot_ids != set(range(29, 45)):
        raise ValueError("OR93 robot/static body partition drifted")
    resources = contract["resource_boundary"]
    expected = {
        "already_extracted_physical_frame_reads_allowed": 6,
        "state_trace_frame_reads_allowed": 6,
        "new_physical_video_decodes_allowed": 0,
        "shared_parameter_count_allowed": 3,
        "analytic_search_candidate_evaluations_allowed": 195,
        "exact_full_mesh_baseline_renders_allowed": 6,
        "exact_full_mesh_selected_renders_allowed": 6,
        "mesh_asset_reads_allowed": 18,
    ...
    if resources != expected or any(contract["authority"].values()):
        raise ValueError("OR93 resource or authority boundary drifted")
    if (
        contract["claim_limits"]["same_video_semantic_match"] is not False
        or contract["claim_limits"]["untouched_cohort_remaining"] is not False
        or contract["claim_limits"]["post_final_retrospective_diagnostic_only"] is not True
    ):
        raise ValueError("OR93 claim boundary drifted")
    return contract


def _quaternion_left_multiply_wxyz(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    lw, lx, ly, lz = left
    rw, rx, ry, rz = right
    value = np.asarray(
        [
...
def _robot_registered_trace(
    ...
    *,
    ...
    """Return a one-frame trace with only the declared robot bodies transformed."""
    if len(trace["frames"]) != 1:
        raise ValueError("OR93 transform expects exactly one trace frame")
    state = trace["frames"][0]
    positions = np.asarray(state["p"], dtype=np.float64).reshape((-1, 3)).copy()
    quaternions = np.asarray(state["q"], dtype=np.float64).reshape((-1, 4)).copy()
    yaw_degrees, translation_x, translation_y = np.asarray(vector, dtype=np.float64)
    ...
    translation = np.asarray([translation_x, translation_y, 0.0], dtype=np.float64)
    anchor = positions[int(anchor_body_id)].copy()
    for body_id in robot_body_ids:
        original = positions[int(body_id)].copy()
        positions[int(body_id)] = anchor + rotation_z @ (original - anchor) + translation
        quaternions[int(body_id)] = _quaternion_left_multiply_wxyz(
            yaw_quaternion, quaternions[int(body_id)]
        )
    transformed_state = dict(state)
    transformed_state["p"] = positions.reshape(-1).tolist()
    transformed_state["q"] = quaternions.reshape(-1).tolist()
    return {"body_names": list(trace["body_names"]), "frames": [transformed_state]}


def _write_png(path: Path, frame: np.ndarray) -> dict[str, Any]:
...
def fit_once(
    ...
    if receipt_path.exists():
        raise ValueError("OR93 one-run receipt already exists")
    contract = load_post_final_shared_robot_base_se2_diagnostic_contract(contract_path)
    or92_closeout = json.loads((REPO_ROOT / contract["sources"]["or92_closeout"]["path"]).read_text())
    if or92_closeout["selected_mechanism"] != "separate_robot_base_and_static_workcell_registration":
        raise ValueError("OR92 did not select the OR93 factorization")
    ...
    for sample in contract["samples"]:
        ...
        key = (recording_id, sample["sample"])
        if key not in pair_map:
            raise ValueError("OR93 sample is not one of the six OR92 frame pairs")
        pair = pair_map[key]
        physical_path = REPO_ROOT / pair["physical"]["path"]
        if sha256_file(physical_path) != pair["physical"]["sha256"]:
            raise ValueError("OR93 physical frame hash mismatch")
        row = frame_map[(recording_id, int(sample["evaluation_index"]))]
        if int(row["state_trace_frame_index"]) != int(sample["state_trace_frame_index"]):
            raise ValueError("OR93 trace-frame binding drifted")
        if recording_id not in trace_cache:
            binding = episode_map[recording_id]["state_trace"]
            trace_path = REPO_ROOT / binding["path"]
            if sha256_file(trace_path) != binding["sha256"]:
                raise ValueError("OR93 state trace hash mismatch")
        ...
        one_trace = {
            ...
            "frames": [trace["frames"][int(sample["state_trace_frame_index"])]],
        }
        physical = cv2.imread(str(physical_path), cv2.IMREAD_COLOR)
        if physical is None or physical.shape[:2] != (240, 320):
            raise ValueError("OR93 physical audit frame is unreadable or wrong size")
        prepared.append({"sample": sample, "physical": physical, "trace": one_trace})
    ...
    robot_ids = [int(value) for value in frozen["baseline_robot_transform"]["transformed_body_ids"]]
    baseline_vector = np.asarray(frozen["baseline_robot_transform"]["vector"], dtype=np.float64)
    ...
    search_physical = [
        cv2.resize(row["physical"], (search_width, search_height), interpolation=cv2.INTER_AREA)
        for row in prepared
    ]
    search_points = np.asarray(contract["regions"]["board_plus_margin"]["points_px"], dtype=np.float64) * np.asarray([search_width / 320.0, search_height / 240.0])
    ...
    def proxy_rows(vector: np.ndarray) -> list[dict[str, float]]:
        values: list[dict[str, float]] = []
        for physical, row in zip(search_physical, prepared, strict=True):
            robot_trace = _robot_registered_trace(
                row["trace"], anchor_body_id=anchor_body_id, robot_body_ids=robot_ids, vector=vector
            )
            candidate = _render_analytic_candidate(
                ...
                width=search_width,
                height=search_height,
                background_rgb=renderer["background_rgb"],
            )
            ...
            whole = _metrics(physical, candidate, edge)
            ...
            values.append(
                {
                    ...
                    "whole_frame_edge_f1": float(whole["tolerant_edge_f1"]),
                    "whole_frame_linear_similarity": float(whole["full_frame_linear_pixel_similarity"]),
                }
            )
        return values

    proxy_baseline_rows = proxy_rows(baseline_vector)
    proxy_baseline_board = _mean(proxy_baseline_rows, "board_plus_margin_edge_f1")
    evaluation_count = 0
    best_score = -np.inf
    ...
    def objective(vector: np.ndarray) -> float:
        ...
        rows = proxy_rows(vector)
        board_mean = _mean(rows, "board_plus_margin_edge_f1")
        score = (
            _mean(rows, "outside_board_edge_f1")
            + 0.15 * _mean(rows, "whole_frame_edge_f1")
            + 0.05 * _mean(rows, "whole_frame_linear_similarity")
            - 0.25 * max(0.0, proxy_baseline_board - board_mean - 0.02)
        )
        evaluation_count += 1
        if score > best_score:
            best_score = score
    ...
    if evaluation_count > int(search["maximum_candidate_evaluations"]):
        raise RuntimeError("OR93 search exceeded frozen candidate budget")
    selected_vector = np.asarray(result.x, dtype=np.float64)
    proxy_selected_rows = proxy_rows(selected_vector)
    ...
    for row in prepared:
        ...
        for label, vector in (("baseline", baseline_vector), ("selected", selected_vector)):
            ...
            pixels, depths, colors, triangle_count = _prepare_full_mesh_stream(
                scene, robot_trace, meshes, camera, renderer, static_family, static_vector
            )
            ...
            physical_gray = cv2.cvtColor(row["physical"], cv2.COLOR_BGR2GRAY)
            candidate_gray = cv2.cvtColor(candidate, cv2.COLOR_BGR2GRAY)
            whole = _metrics(row["physical"], candidate, edge)
            ...
            filename = f"{row['sample']['recording_id']}-{row['sample']['sample']}-{label}.png"
            variants[label] = {
                "whole_frame": whole,
                "board_plus_margin": board,
                "outside_board": outside,
                "render": {
                    "total_raster_triangle_count": int(triangle_count),
                    "depth_buffer_update_count": int(updates),
                    "occluded_fragment_count": int(occluded),
                    "native_raster_seconds": float(raster_seconds),
                },
        ...
        final_rows.append(
            {
                "recording_id": row["sample"]["recording_id"],
                "sample": row["sample"]["sample"],
                "state_trace_frame_index": int(row["sample"]["state_trace_frame_index"]),
        ...
        montage_rows.append(np.concatenate([row["physical"], images["baseline"], images["selected"]], axis=1))

    montage = np.concatenate(montage_rows, axis=0)
    montage_binding = _write_png(output_directory / "physical_baseline_selected.png", montage)
    ...
    gates = {
        ...
        "one_shared_three_parameter_robot_vector": len(selected_vector) == 3,
        "analytic_search_within_frozen_budget": evaluation_count <= int(search["maximum_candidate_evaluations"]),
        "exact_six_baseline_and_six_selected_full_mesh_renders": len(triangle_counts) == 12,
        "expected_triangle_count_every_render": all(value == 824944 for value in triangle_counts),
        ...
        "no_new_physical_decode_replay_hardware_or_paid_compute": True,
        "post_final_diagnostic_not_promotion": True,
    }
    passed = all(gates.values())
    receipt: dict[str, Any] = {
        "schema_version": "sim2claw.observable_registration_post_final_shared_robot_base_se2_diagnostic_receipt.v1",
        "experiment_id": contract["experiment_id"],
        "status": "PASS_SHARED_ROBOT_BASE_SE2_HEADROOM_SELECTED" if passed else "TERMINAL_SHARED_ROBOT_BASE_SE2_INSUFFICIENT",
        "proof_class": contract["proof_class"],
        "contract": {"path": str(contract_path.relative_to(REPO_ROOT)), "sha256": sha256_file(contract_path)},
        "selected": {
            "parameter_names": contract["robot_registration_family"]["parameter_names"],
            "vector": selected_vector.tolist(),
            "optimizer_converged": bool(result.success),
            "optimizer_message": str(result.message),
            "proxy_rows": proxy_selected_rows,
        },
        "baseline": {"vector": baseline_vector.tolist(), "proxy_rows": proxy_baseline_rows},
        ...
        "reviewer_decision": "FREEZE_POST_FINAL_SHARED_ROBOT_BASE_FULL_CORPUS_DIAGNOSTIC" if passed else "REJECT_SHARED_ROBOT_BASE_AND_FREEZE_INDEPENDENT_LEFT_RIGHT_DIAGNOSTIC",
        "next_transition": "freeze_or94_post_final_shared_robot_base_full_corpus_diagnostic" if passed else "freeze_or94_post_final_independent_left_right_robot_base_diagnostic",
    }

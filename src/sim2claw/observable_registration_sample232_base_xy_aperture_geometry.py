"""Bounded static base-XY and aperture enclosure geometry diagnostic."""
...
from .observable_registration_sample232_aperture_geometry import (
    _minimum_gap,
    load_contract as load_or29_contract,
)
...
SCHEMA = "sim2claw.observable_registration_sample232_base_xy_aperture_geometry_contract.v1"
RECEIPT_SCHEMA = "sim2claw.observable_registration_sample232_base_xy_aperture_geometry_receipt.v1"
CONTRACT_PATH = REPO_ROOT / "configs/evaluations/observable_registration_sample232_base_xy_aperture_geometry_v1.json"
OUTPUT_DIRECTORY = REPO_ROOT / "outputs/observable_registration_sample232_base_xy_aperture_geometry_v1"
...
def load_contract(
    ...
    contract = load_json_object(path, label="sample-232 base XY aperture geometry")
    ...
    or29, or28, c6 = load_or29_contract(
        _bound_path(contract["sources"]["or29_contract"], root=root, label="OR29"),
        root=root,
    )
    axis = contract["grid"]["left_base_world_x_delta_m"]
    _require(
        axis == contract["grid"]["left_base_world_y_delta_m"]
        and len(axis) == 9
        and axis[0] == -0.02
        and axis[-1] == 0.02,
        "base translation grid changed",
    )
    ...
    _require(
        ...
        and evaluation["dynamic_outcomes_may_not_select_grid_row"] is True,
    ...
    return contract, or29, or28, c6


def evaluate_sample232_base_xy_aperture_geometry(
    *,
    ...
    _require(not receipt_path.exists(), "OR30 receipt already exists")
    contract, or29, or28, c6 = load_contract(contract_path, root=root)
    source = c6["source"]
    ...
    historical = _bound_json(
        or28["sources"]["historical_mapping_receipt"], root=root, label="mapping"
    )
    ...
    original_base_xy = model.body_pos[left_base, :2].copy()
    joint_ids = [
    ...
    data.qpos[qpos_addresses] = physical_to_model(
        measured[:1], {"candidate_config": base_candidate}
    )[0]
    ...
    guard_first, guard_last = contract["selection"]["precontact_guard_interval"]
    ...
    for dx in contract["grid"]["left_base_world_x_delta_m"]:
        for dy in contract["grid"]["left_base_world_y_delta_m"]:
            model.body_pos[left_base, :2] = original_base_xy + np.asarray([dx, dy])
            for offset in or29["grid"]["gripper_zero_offset_rad"]:
                candidate = copy.deepcopy(base_candidate)
                candidate["physical_adapter"]["joint_transform"]["joints"][5]["zero_offset"] = float(offset)
                applied_model = physical_to_model(applied_physical, {"candidate_config": candidate})
                gaps = []
                for index in range(guard_first, sample + 1):
                    ...
                    gaps.append(
                        (
                            index,
                            _minimum_gap(model, data, fixed, pawn),
                            _minimum_gap(model, data, moving, pawn),
                        )
                    )
                final = gaps[-1]
                penetration = max(
                    0.0,
                    -min(min(fixed_gap, moving_gap) for _, fixed_gap, moving_gap in gaps[:-1]),
                )
                eligible = (
                    lower <= final[1] <= upper
                    and lower <= final[2] <= upper
                    and penetration <= float(contract["selection"]["maximum_precontact_penetration_m"])
                )
                rows.append(
                    {
                        "left_base_world_x_delta_m": float(dx),
                        "left_base_world_y_delta_m": float(dy),
                        "translation_norm_m": float(np.hypot(dx, dy)),
                        ...
                        "score": max(abs(final[1]), abs(final[2])),
                        "eligible": eligible,
                    }
                )
    ...
    receipt = {
        ...
        "status": "PASS_STATIC_BASE_XY_APERTURE_CANDIDATE" if selected else "NO_STATIC_BASE_XY_APERTURE_CANDIDATE",
        "source_identity": {
        ...
        "base_translation_exceeds_metrology_uncertainty_if_selected": (
            bool(selected and selected["translation_norm_m"] > 0.0027)
        ),

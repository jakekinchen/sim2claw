"""Post-terminal control for the exhausted C922 calibration evaluation."""
...
EXPECTED_GUARD = {
    "schema_version": "sim2claw.c922_exact_mode_calibration_exhaustion.v1",
    "contract_id": "current_100mm_c922_exact_mode_calibration_v1",
    "status": "terminal_dataset_evaluation_exhausted_corpus_missing",
    "contract_sha256": (
    ...
    "execution_commit": "e32553ebc2682fa18e5c531b4b1404b073d4c6d8",
    "evaluator_sha256": (
    ...
    "verdict": "calibration_dataset_not_ready",
    ...
    "calibration_receipt_emitted": False,
...
def load_exhaustion_guard(path: Path = GUARD_PATH) -> dict[str, object]:
    try:
        ...
        raise C922CalibrationError(
            f"C922 calibration exhaustion guard is unavailable: {error}"
        ) from error
    if payload != EXPECTED_GUARD:
        raise C922CalibrationError("C922 calibration exhaustion guard changed.")
    return payload
...
def run_authorized_evaluation(*_: object, **__: object) -> NoReturn:
    ...
    raise C922CalibrationError(
        "C922 exact-mode calibration v1 is exhausted; use a new transaction."
    )

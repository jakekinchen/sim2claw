"""Propagate OR127's frozen procedural fixture through the OR119 timeline."""
...
SCHEMA = "sim2claw.observable_registration_renderer_native_planar_fixture_full_timeline_propagation_contract.v1"
DEFAULT_CONTRACT = REPO_ROOT / "configs/evaluations/observable_registration_renderer_native_planar_fixture_full_timeline_propagation_v1.json"
DEFAULT_OUTPUT = REPO_ROOT / "outputs/observable_registration_renderer_native_planar_fixture_full_timeline_propagation_v1"
...
def load_planar_fixture_full_timeline_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    contract = json.loads(path.read_text())
    if contract.get("schema_version") != SCHEMA:
        raise ValueError("unsupported OR128 contract")
    for group in ("sources", "frozen_identities"):
        for binding in contract[group].values():
            ...
            if source_path and expected and sha256_file(REPO_ROOT / source_path) != expected:
                raise ValueError(f"OR128 identity mismatch: {source_path}")
    fixture = contract["fixture"]
    if fixture["triangle_count"] != 128 or fixture["shared_zbuffer"] is not True:
        raise ValueError("OR128 fixture geometry drifted")
    if fixture["physical_pixel_texture_projection"] is not False or fixture["screen_space_overlay"] is not False:
        raise ValueError("OR128 texture or overlay boundary drifted")
    ...
    if resources["physical_frames_compared_allowed"] != 1210 or resources["candidate_frames_rendered_allowed"] != 1210 or resources["candidate_videos_allowed"] != 11:
        raise ValueError("OR128 corpus budget drifted")
    ...
    if any(resources[key] != 0 for key in zero) or resources["paid_compute_allowed"] is not False:
        raise ValueError("OR128 resource boundary drifted")
    if any(contract["authority"].values()):
        raise ValueError("OR128 authority must remain closed")
    return contract
...
def _merged_or119_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    merged["gates"]["expected_total_raster_triangle_count_per_frame"] = int(contract["gates"]["expected_total_raster_triangle_count_per_frame"])
...
def evaluate_once(contract_path: Path = DEFAULT_CONTRACT, output_directory: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    if receipt_path.exists():
        raise ValueError("OR128 one-run receipt already exists")
    contract = load_planar_fixture_full_timeline_contract(contract_path)
    predecessor = json.loads((REPO_ROOT / contract["sources"]["or127_receipt"]["path"]).read_text())
    parameters = json.loads((REPO_ROOT / contract["sources"]["or126_parameters"]["path"]).read_text())
    if predecessor["artifact_sha256"] != contract["sources"]["or127_receipt"]["artifact_sha256"] or predecessor["status"] != "PASS_RENDERER_NATIVE_PLANAR_FIXTURE_STATIC_COMPARISON":
        raise ValueError("OR128 OR127 prerequisite drifted")
    if parameters["artifact_sha256"] != contract["sources"]["or126_parameters"]["artifact_sha256"]:
        raise ValueError("OR128 fixture parameterization drifted")
    ...
    def prepare_with_fixture(*args: Any, **kwargs: Any) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    ...
    try:
        ...
        _or119._prepare_full_mesh_stream = prepare_with_fixture
    ...
    receipt["schema_version"] = "sim2claw.observable_registration_renderer_native_planar_fixture_full_timeline_propagation_receipt.v1"
    receipt["identities"] = {
    ...
    receipt["fixture"] = {
        "parameterization": contract["sources"]["or126_parameters"],
        "triangle_count": len(fixture_pixels),
        "shared_zbuffer": True,

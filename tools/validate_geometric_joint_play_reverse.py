#!/usr/bin/env python3
"""Evaluate the frozen reverse-play fit on one fresh geometric heldout."""
...
CONTRACT_PATH = (
    ...
    / "configs"
    / "evaluations"
    / "geometric_joint_play_reverse_holdout_v1.json"
)
SCHEMA = "sim2claw.geometric_joint_play_reverse_holdout.v1"
RECEIPT_SCHEMA = "sim2claw.geometric_joint_play_reverse_holdout_receipt.v1"


class GeometricJointPlayReverseHoldoutError(RuntimeError):
    """The reverse fit, heldout, evaluator, or authority boundary changed."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise GeometricJointPlayReverseHoldoutError(message)
...
def validate(
    *,
    ...
    _require(
        ...
        "frozen reverse fit selection changed",
    )

    parent_contract_path = _bound_path(
        fit_contract["sources"]["parent_fit_contract"]
    )
    parent_contract = _load_json(parent_contract_path)
    parent_receipt_path = _bound_path(
        fit_contract["sources"]["parent_fit_receipt"]
    )
    parent_receipt = _load_json(parent_receipt_path)
    _verify_digest(
        parent_receipt,
        str(
            fit_contract["sources"]["parent_fit_receipt"]["receipt_digest"]
        ),
        label="parent fit",
    )
    _require(
        parent_receipt.get("contract_sha256")
        == sha256_file(parent_contract_path)
        and parent_receipt.get("selected_expanded_half_width_degrees")
        == expected_fit["parent_expanded_half_width_degrees"],
        "frozen parent fit changed",
    )

    selection_path = _bound_path(
        parent_contract["sources"]["selection_receipt"]
    )
    selection = _load_json(selection_path)
    _verify_digest(
        selection,
        str(
            parent_contract["sources"]["selection_receipt"]["receipt_digest"]
        ),
        label="selection",
    )
    mechanism_path = _bound_path(
        parent_contract["sources"]["mechanism_contract"]
    )
    ...
    threshold = float(
        parent_contract["mechanism"]["load_sign_zero_threshold_nm"]
    )
    ...
    parent_states, parent_schedule = replay_joint_play(
        ...
        half_width_degrees=expected_fit[
            "parent_expanded_half_width_degrees"
        ],
        load_sign_zero_threshold_nm=threshold,
    )
    ...
    receipt = {
        ...
        "verdict": (
            "reverse_joint_play_passed_fresh_geometric_heldout"
            if passed
            else "reverse_joint_play_rejected_on_fresh_geometric_heldout"
        ),

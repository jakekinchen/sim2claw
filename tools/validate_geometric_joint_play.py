#!/usr/bin/env python3
"""Evaluate frozen joint play on one preregistered geometric heldout."""
...
CONTRACT_PATH = (
    ...
    / "configs"
    / "evaluations"
    / "geometric_joint_play_holdout_v1.json"
)
SCHEMA = "sim2claw.geometric_joint_play_holdout.v1"
RECEIPT_SCHEMA = "sim2claw.geometric_joint_play_holdout_receipt.v1"


class GeometricJointPlayHoldoutError(RuntimeError):
    """The fit, heldout, evaluator, or authority boundary changed."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise GeometricJointPlayHoldoutError(message)
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise GeometricJointPlayHoldoutError(
...
def _bound_path(binding: Mapping[str, Any]) -> Path:
    ...
    _require(
        ...
        f"hash-bound source changed: {path.relative_to(REPO_ROOT)}",
    )
    return path
...
def validate(
    *,
    ...
    boundary = contract.get("proof_boundary") or {}
    _require(
        boundary
        == {
    ...
    fit_digest_payload = dict(fit_receipt)
    observed_fit_digest = fit_digest_payload.pop("receipt_digest", None)
    expected_fit = contract["expected_fit"]
    _require(
        observed_fit_digest
        == sources["fit_receipt"]["receipt_digest"]
        == canonical_digest(fit_digest_payload)
        and fit_receipt.get("contract_sha256") == sha256_file(fit_contract_path)
        and fit_receipt.get("selected_half_width_degrees")
        == expected_fit["selected_half_width_degrees"]
        and fit_receipt.get("selected_expanded_half_width_degrees")
        == expected_fit["selected_expanded_half_width_degrees"]
    ...
    observed_selection_digest = selection_digest_payload.pop(
        "receipt_digest", None
    )
    _require(
        observed_selection_digest
    ...
    _require(
        packet_stage["action_sha256"] == stage_spec["expected_action_sha256"]
    ...
    baseline_states, baseline_schedule = _timestamp_aligned_zoh(
        mapped,
        workcell,
        ...
        servo_deadband_degrees={
            ...
            for key, value in contract[
                "baseline_memoryless_deadband_degrees"
            ].items()
        },
    )
    selected_states, selected_schedule = replay_joint_play(
        ...
        settle_steps=settle_steps,
        delay_seconds=delay_seconds,
        half_width_degrees=expected_fit[
            "selected_expanded_half_width_degrees"
        ],
        load_sign_zero_threshold_nm=float(
            fit_contract["mechanism"]["load_sign_zero_threshold_nm"]
        ),
    )
    baseline_metrics = _strip_arrays(
        _episode_metrics(mapped, baseline_states, workcell, mechanism)
    )
    ...
    gates = {
        ...
        "selected_joint_rms_improves_over_baseline": (
            comparison["joint_rms_relative_improvement"] > 0.0
        ),
        "selected_ee_rms_improves_over_baseline": (
    ...
    receipt = {
        ...
        "variants": {
            ...
            "selected_stateful_play": {
                "parameters": expected_fit[
                    "selected_expanded_half_width_degrees"
                ],
        ...
        "verdict": (
            "stateful_joint_play_passed_fresh_geometric_heldout"
            if passed
            else "stateful_joint_play_rejected_on_fresh_geometric_heldout"
        ),

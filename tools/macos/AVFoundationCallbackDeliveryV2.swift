...
private let observationSchema = "sim2claw.avfoundation_c922_callback_observation.v2"
private let eventSchema = "sim2claw.avfoundation_c922_callback_event.v2"
...
private func formatIdentity(
    ...
    let exact = dimensions.width == options.width
        ...
        && minimum.isFinite
        && maximum.isFinite
        && abs(minimum - (1.0 / options.supportedFPS)) < 1e-9
        && abs(maximum - (1.0 / options.supportedFPS)) < 1e-9
...
private func finish(
    ...
    try writeObservation(
        Observation(
            ...
            captureSessionsUsed: captureSessionsUsed,
            events: final.events
        ),
...
private func run(_ options: Options) throws {
    ...
    let queue = DispatchQueue(label: "sim2claw.avfoundation-c922-callback-delivery-v2")
    ...
    let associated = formatIdentity(device: device, options: options, preset: session.sessionPreset)
    ledger.emit("format_after_input_association", update: associated.update)

    session.commitConfiguration()
    let committed = formatIdentity(device: device, options: options, preset: session.sessionPreset)
    ledger.emit("format_after_commit", update: committed.update)
    guard associated.exact && committed.exact else {
        try finish(options: options, ledger: ledger, captureSessionsUsed: 1)
        return
    }

    session.startRunning()
    ledger.emit("session_start_returned") { $0.sessionRunning = session.isRunning }
    let started = formatIdentity(device: device, options: options, preset: session.sessionPreset)
    ledger.emit("format_after_start", update: started.update)
    if session.isRunning && started.exact {
...
do {
    ...
    FileHandle.standardError.write(
        Data("AVFoundationCallbackDeliveryV2: \(error)\n".utf8)
    )
    exit(2)
}

...
private let observationSchema = "sim2claw.avfoundation_c922_callback_observation.v1"
private let eventSchema = "sim2claw.avfoundation_c922_callback_event.v1"
...
private enum ObserverFailure: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .sessionStart: return "capture_session_failed_to_start"
        case .noSamples: return "no_source_callbacks"
...
private struct Options {
    ...
    static func parse(_ arguments: [String]) throws -> Options {
        ...
        guard unexpected.isEmpty else {
            throw ObserverFailure.invalidArguments(
        ...
        guard
            ...
            let formatIndexText = values["--format-index"],
            let formatIndex = Int(formatIndexText), formatIndex >= 0,
            let rangeIndexText = values["--range-index"],
            let rangeIndex = Int(rangeIndexText), rangeIndex >= 0,
            ...
            let maximumText = values["--maximum-callbacks"],
            let maximumCallbacks = Int(maximumText), maximumCallbacks > 0,
            let contractSHA256 = values["--contract-sha256"],
            contractSHA256.count == 64,
            let outputPath = values["--output"], !outputPath.isEmpty
        else {
            throw ObserverFailure.invalidArguments("Required callback-delivery arguments are invalid.")
        }
        return Options(
...
private final class ContinuousClock {
    ...
    func nowNS() -> UInt64 {
        ...
        return UInt64((Double(ticks) * Double(numer)) / Double(denom))
    }
}
...
private final class EventLedger: @unchecked Sendable {
    ...
    func emitSample(
        ...
        let pixelFormat = pixelBuffer.map { fourCC(CVPixelBufferGetPixelFormatType($0)) }
        var event = CallbackEvent(
            eventIndex: rows.count,
            eventType: type,
            hostContinuousNS: clock.nowNS()
...
private func run(_ options: Options) throws {
    ...
    guard subtype == options.subtype else {
        throw ObserverFailure.formatIdentity("subtype=\(subtype)")
    }
    guard abs(range.maxFrameRate - options.supportedFPS) < 1e-9,
          abs(range.minFrameRate - options.supportedFPS) < 1e-9
    else {
        throw ObserverFailure.formatIdentity(
            "range=\(range.minFrameRate)...\(range.maxFrameRate)"
        )
    }
    ...
    guard activeDimensions.width == options.width,
          ...
          activeSubtype == options.subtype
    else {
        throw ObserverFailure.formatIdentity("active format changed")
    }
    ledger.emit("format_applied") {
        ...
        $0.formatWidth = Int(activeDimensions.width)
        $0.formatHeight = Int(activeDimensions.height)
        $0.formatMediaSubtype = activeSubtype
        $0.supportedFPS = range.maxFrameRate
        $0.frameDurationSeconds = CMTimeGetSeconds(range.minFrameDuration)
    }
    ...
    let queue = DispatchQueue(label: "sim2claw.avfoundation-c922-callback-delivery-v1")
    ...
    guard session.isRunning else { throw ObserverFailure.sessionStart }
    ...
    guard beforeFinish.output > 0 else { throw ObserverFailure.noSamples }
    ...
    try writeObservation(observation, path: options.outputPath)
}
...
do {
    ...
    FileHandle.standardError.write(
        Data("AVFoundationCallbackDeliveryV1: \(error)\n".utf8)
    )
    exit(2)
}

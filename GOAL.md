...
Status: `OR10 ZERO-NEW-DATA C922 PIXEL REFINEMENT FROZEN`
...
OR10 is now the one active bounded retained-data card. The prior `2.270 px`
...
across cohorts. Because its extraction protocol was developed after inspecting
the fit images and no pristine pose remains, it can improve the inspection-only
board-plane camera but cannot approve exact intrinsics, distortion, global
mapping, contact, task transfer, or canonical simulator-camera replacement.
